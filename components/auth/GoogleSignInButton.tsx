'use client'

import { useState } from 'react'
import { Loader2 } from 'lucide-react'
import { useAuth } from '@/contexts/AuthContext'
import { isFirebaseConfigured } from '@/lib/firebase'
import { cn } from '@/lib/utils'

interface GoogleSignInButtonProps {
  label?: string
  onError?: (message: string) => void
  disabled?: boolean
}

function GoogleIcon() {
  return (
    <svg className="h-[18px] w-[18px] shrink-0" viewBox="0 0 24 24" aria-hidden="true">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  )
}

export function GoogleSignInButton({
  label = 'Continue with Google',
  onError,
  disabled = false,
}: GoogleSignInButtonProps) {
  const { loginWithGoogle } = useAuth()
  const [loading, setLoading] = useState(false)

  if (!isFirebaseConfigured()) {
    return null
  }

  const handleClick = async () => {
    setLoading(true)
    try {
      await loginWithGoogle()
    } catch (err: unknown) {
      const code =
        err && typeof err === 'object' && 'code' in err
          ? String((err as { code: string }).code)
          : ''
      const message = err instanceof Error ? err.message : 'Google sign-in failed'
      if (code === 'auth/popup-closed-by-user' || message.includes('popup-closed')) {
        onError?.('Sign-in was cancelled')
      } else {
        onError?.(message)
      }
    } finally {
      setLoading(false)
    }
  }

  return (
    <button
      type="button"
      onClick={handleClick}
      disabled={disabled || loading}
      className={cn(
        'w-full h-11 rounded-xl flex items-center justify-center gap-3',
        'bg-white/[0.06] hover:bg-white/[0.1] border border-white/10 hover:border-white/20',
        'text-white text-sm font-medium',
        'transition-all hover:scale-[1.01] active:scale-[0.99]',
        'disabled:opacity-50 disabled:pointer-events-none',
        'shadow-sm'
      )}
    >
      {loading ? (
        <Loader2 className="h-[18px] w-[18px] animate-spin text-slate-400" />
      ) : (
        <GoogleIcon />
      )}
      <span>{loading ? 'Connecting...' : label}</span>
    </button>
  )
}

export function AuthDivider() {
  return (
    <div className="relative my-6">
      <div className="absolute inset-0 flex items-center">
        <div className="w-full border-t border-white/8" />
      </div>
      <div className="relative flex justify-center">
        <span className="bg-[#12121a] px-3 text-xs text-slate-500 uppercase tracking-widest">
          or continue with email
        </span>
      </div>
    </div>
  )
}
